...
const SeatGrid = ({ selectedArray, onToggle, forceAll, activeColor }) => (
  <div style={{ background: '#fafafa', padding: '15px', borderRadius: '10px', border: '1px solid #eee' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 0.4fr 1fr 1fr 1fr', gap: '8px' }}>
      {Array.from({ length: 30 }, (_, i) => i + 1).map(num => {
        ...
        return (
          <div key={num} onClick={() => !forceAll && onToggle(num)}
            style={{
              gridColumn: gridColumn,
              padding: '10px 0', textAlign: 'center', borderRadius: '6px',
              fontWeight: '700', fontSize: '0.9rem',
              cursor: forceAll ? 'not-allowed' : 'pointer',
              userSelect: 'none',
              background: on ? activeColor : 'white',
              color: on ? 'white' : '#64748b',
              border: on ? `1px solid ${activeColor}` : '1px solid #cbd5e1',
