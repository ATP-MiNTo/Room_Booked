    ...
    try:
        ...
        if existing_student:
            ...
            if db_fname != first_name or db_lname != last_name or db_major != major:
                raise HTTPException(status_code=400, detail="ข้อมูล ชื่อ, นามสกุล หรือ สาขา ไม่ตรงกับรหัสนักศึกษานี้ในระบบ")
        else:
...
def sync_attendance(body: AttendanceSyncRequest):
    ...
    try:
        ...
        for seat in body.seats:
            ...
            status = "present" if is_sitting else None
...
def get_all_reservations():
    ...
    try:
        ...
        return [
            {
                "id": r[0], "seat_id": r[1],
                "reserve_date": str(r[2].date()) if r[2] else None,
                "start_time": str(r[2].time()) if r[2] else None,
                "end_time": str(r[3].time()) if r[3] else None,
                "student_id": r[4], "image_filename": r[5],
                "first_name": r[6] or "ไม่ระบุ", "last_name": r[7] or "",
                "major": r[8] or "ไม่ระบุ", "purpose": r[9] or "ไม่ระบุ",
...
def get_analytics_kpi(period: str = Query("week"), ref_date: str = Query(None), ref_week: str = Query(None), ref_month: str = Query(None), ref_year: str = Query(None)):
    ...
    try:
        ...
        if period == "today":
            d = datetime.strptime(ref_date, "%Y-%m-%d") if ref_date else now
...
def get_seat_usage(start: str = Query(None), end: str = Query(None), month: str = Query(None), year: str = Query(None)):
    ...
    try:
        ...
        cur.execute(f"SELECT seat_no, COUNT(*) as usage FROM reservations WHERE 1=1{clause} GROUP BY seat_no ORDER BY usage DESC", params)
        rows = cur.fetchall()
...
def get_peak_hours(start: str = Query(None), end: str = Query(None), month: str = Query(None), year: str = Query(None)):
    ...
    try:
        ...
        params = []
        clause = _build_date_filter(params, month=month, year=year, start=start, end=end)
        cur.execute(f"SELECT EXTRACT(HOUR FROM start_time) as hour, COUNT(*) as users FROM reservations WHERE 1=1{clause} GROUP BY hour ORDER BY hour", params)
        rows = cur.fetchall()
